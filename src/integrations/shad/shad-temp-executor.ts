...
export interface ShadTempVaultExecutorConfig {
  /** The encrypted vault to read from (VaultStore or legacy vault types) */
  vault: VaultStore | LocalEncryptedVault | EncryptedShadVault;
...
export class ShadTempVaultExecutor {
   ...
   */
  private getVaultReader(): Pick<VaultStore, 'read' | 'list'> {
    // VaultStore (preferred path — adapters already conform)
    if ('read' in this.vault && 'list' in this.vault && !('readDocument' in this.vault)) {
      return this.vault as VaultStore;
    }

    // Legacy: EncryptedShadVault (has readDocument method)
    ...
    // LocalEncryptedVault implements VaultStore
    return this.vault as unknown as Pick<VaultStore, 'read' | 'list'>;
  }
