...
export class AGIdentityStorageManager {
  ...
  private async resolveUhrpUrl(uhrpUrl: string): Promise<string[]> {
    ...
    const lookupUrl = getConfig().uhrpStorageUrl;
  ...
  private async createBlockchainTimestamp(
    ...
    // Record for later timestamp verification lookups
    this.timestampTxIndex.set(uhrpUrl, result.txid);

    return result.txid;
  }

  private timestampTxIndex = new Map<string, string>();

  /**
   * Record a timestamp txid for a UHRP URL (called after createBlockchainTimestamp)
   */
  recordTimestampTx(uhrpUrl: string, txid: string): void {
    this.timestampTxIndex.set(uhrpUrl, txid);
  }

  private async queryTimestampTransaction(uhrpUrl: string): Promise<{
    timestamp: number;
    blockHeight: number;
    txId: string;
  } | null> {
    ...
    const txid = this.timestampTxIndex.get(uhrpUrl);
    if (!txid) return null;
    ...
    try {
      ...
      const data = await response.json() as { blockTime?: number; blockHeight?: number };
      if (!data.blockTime || !data.blockHeight) return null;

      return {
        timestamp: data.blockTime * 1000,
